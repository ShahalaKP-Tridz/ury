...
export const Input = (props: React.InputHTMLAttributes<HTMLInputElement>) => (
  <BaseInput 
    {...props}
    className={cn(
      "bg-muted/50 border-input focus:ring-primary/20 rounded-xl px-4 py-3 h-auto transition-all",
      props.className
    )}
  />
);

export const Select = ({ options, value, onChange, placeholder }: { options: { label: string; value: string }[], value: string, onChange: (val: string) => void, placeholder?: string }) => (
  <select 
    value={value}
